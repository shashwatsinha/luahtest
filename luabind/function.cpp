...
namespace
{
    ...
    inline bool is_luabind_function(adl::object const& obj)
  {
...
LUABIND_API void add_overload(
                              adl::object const& context, char const* name, adl::object const& fn)
{
    function_object* f = *touserdata<function_object*>(getupvalue(fn, 1));
    f->name = name;

    if (adl::object overloads = context[name])
    {
    ...
    LUABIND_API adl::object make_function_aux(lua_State* L, function_object* impl)
{
    ...
    *(function_object**)storage = impl;
    ...
    return adl::object(from_stack(L, -1));
}
